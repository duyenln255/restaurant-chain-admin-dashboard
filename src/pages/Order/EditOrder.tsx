...
import type { OrderUpdateRequest } from "../../services/order.service.tsx";
...
const EditOrder: React.FC = () => {
  ...
  const [orderType, setOrderType] = useState<string>("AT STORE");
  ...
  useEffect(() => {
    if (selectedOrder) {
      setCustomerPhone(selectedOrder.phone || "");
      setCustomerName(selectedOrder.name);
      ...
      if (
        selectedOrder.cart &&
        selectedOrder.cart.items &&
        selectedOrder.cart.items.length > 0
      ) {
        const item = selectedOrder.cart.items[0];
  ...
  if (!selectedOrder && !loading)
    return <div className="p-4 text-red-500">{t("orders.noOrdersFound")}</div>;
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                      <label className="block text-sm font-medium mb-1">
                        {t("orders.customer")} {t("orders.phone")}{" "}
