...
const EditBrandManager: React.FC = () => {
  ...
  useEffect(() => {
    const fetchEmployee = async () => {
      try {
        ...
        console.error("Failed to fetch brand manager:", error);
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success("Brand Manager updated successfully!");
      navigate("/brand-manager");
    } catch (error) {
      toast.error("Failed to update brand manager.");
  ...
  return (
    <div className="p-6 mx-auto space-y-6">
      ...
      <h1 className="text-3xl font-bold text-neutral-800">Edit Brand Manager</h1>
      ...
      <div className="bg-white rounded-xl p-8 shadow-md">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="text-center pt-4">
            <button type="submit" className="bg-blue-500 text-white px-8 py-2 rounded-md hover:bg-blue-600">Update</button>
          </div>
        </form>
      </div>
    </div>
