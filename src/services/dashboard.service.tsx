...
export const getSaleReport = async (): Promise<SaleReport[]> => {
  const response = await axiosInstance.get<{ sale_report: SaleReport[] }>(
    "/sale_report"
  );
...
export const getDashboardStats = async (brandId?: string) => {
  ...
  return {
    totalCustomers: brand.total_customers,
    totalOrders: current.total_orders,
    totalSales: current.total_revenue,
    totalStaff: current.total_staffs,
    customerGrowth: calculateGrowth(brand.total_customers, brand.total_customers), // fix as needed
